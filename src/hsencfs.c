...

{
    ...
    printf("            -a prog  (--askpass ) Use program to ask pass. \n");
...

{
    ...
    if(bb > 2)
        ...
        if (verbose)
            fprintf(stderr,
                "Cannot mount, directory not empty or mounted already.\n");
        exit(5);
        }
    if(loglevel > 0)
        {
    ...
    if (access(mountpoint, W_OK) < 0)
        {
        hslog(1, "No mountpoint access, fixing.\n");
